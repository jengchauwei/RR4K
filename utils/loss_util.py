...
class multi_VGGPerceptualLoss(torch.nn.Module):
    def __init__(self, lam=1, lam_p=1):
        ...
        self.loss_fn = VGG19PerceptualLoss()
        self.lam = lam
        self.lam_p = lam_p
...
class VGG19PerceptualLoss(torch.nn.Module):
    def __init__(self, resize=True,requires_grad=False):
        ...
        self.resize = resize
    def forward(self, input, target ,feature_layers=[2, 7, 12, 21, 30]):
        ...
        for i in range(feature_layers[-1]):
            x = self.vgg_pretrained_features[i](x)
            y = self. vgg_pretrained_features[i](y)
            if (i+1) in feature_layers:
